...
??? info "邮箱收到验证邮件了，但点进去后又提示重新登录，导致验证邮件反复发送，怎么办？"

    这是因为邮件里的验证链接在打开时没有带上你已登录的会话，系统会认为你未登录，于是又触发一次验证邮件。

    解决方案：
    
    1. 先在浏览器中打开 [Geocaching](https://www.geocaching.com/account/signin)，并登录你的新账号（确保处于登录状态）。
    2. 回到邮箱，打开验证邮件，长按验证按钮，选择「复制链接地址」。
    3. 将复制的链接粘贴到刚才已登录账号的浏览器地址栏中打开。
...
另外也可以试试热心宝友制作的第三方地图网站 [tftc.top](https://tftc.top/)（使用指南 [见此](./tftc.md)）。

**对新手来说，直接通过手机浏览器访问藏宝地图会更加方便和简单，配置步骤也更少，适合完成第一次寻宝。**
...
- Android：推荐 **c:geo**。可下载群友制作的 [国内地图版 APK](https://geoguide.top/cgeo-cn.apk)，支持高德、腾讯和卫星图；也可访问 [c:geo 官网](https://www.cgeo.org/) 或下载 [原版 APK](https://www.freemindworld.com/tmp/cgeo-release_latest.apk)。
    - 具体的使用说明见：[c:geo 使用指南](./cgeo.md)
    - 国内地图版可选择高德、腾讯或卫星图
    - 原版地图源建议 `OpenStreetMap.de`
...
!!! warning "中国国内地图的经纬度坐标偏移问题"
    由于国家测绘法规要求，在国家测绘法规的要求下，国内所有公开地图（如高德、百度）均不能直接使用国际通用的 **WGS-84 坐标系**，而是采用其他坐标体系。除百度地图使用 **BD-09 坐标系** 外，大多数地图服务使用 **GCJ-02 坐标系**。这些坐标系相对于 WGS-84 会产生约 **300–500 米的偏移**，也就是常说的「火星坐标系」。这意味着：
    ...
    - [坐标转换小工具](https://tools.geocaching.top/)
...
!!! note "拿到坐标后，怎么快速确定具体位置？"

    用 [坐标转换小工具](https://tools.geocaching.top/) 可以快速把坐标打开到常用地图软件中。
